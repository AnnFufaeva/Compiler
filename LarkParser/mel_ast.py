...
class TypeNode(IdentNode):
    ...
    def __init__(self, name: str, num: LiteralNode = None,
                 row: Optional[int] = None, col: Optional[int] = None, **props) -> None:
        ...
        if num is None:
            with suppress(SemanticException):
                self.type = TypeDesc.from_str(name, True)
        else:
            self.num = num
            with suppress(SemanticException):
                self.type = TypeDesc.from_str(name, False)
...
class AssignNode(ExprNode):
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        var_ = scope.get_ident(self.var.name)
        self.val = type_convert(self.val, self.var.node_type, self, 'присваиваемое значение')
        if var_.array_num is not None:
            if len(self.val.params) != var_.array_num:
                self.semantic_error(
                    'Количество элементов в массиве {} не совпадает с заданным {}'.format(len(self.val.params),
                                                                                var_.array_num))
...
class VarsNode(StmtNode):
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        for var in self.vars:
            ...
            try:
                if self.type.num is None:
                    scope.add_ident(IdentDesc(var_node.name, self.type.type))
                else:
                    scope.add_ident(IdentDesc(var_node.name, self.type.type, array_num=self.type.num.value))
...
class ArrNode(ExprNode):
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        for i in range(len(self.params)):
            param: ExprNode = self.params[i]
            param.semantic_check(scope)
            if param.node_type == TypeDesc.BOOL:
                if flag:
                    self.node_type = TypeDesc.BOOL_ARRAY
                    flag = False
                elif self.node_type != TypeDesc.BOOL_ARRAY:
                    self.semantic_error('Элементы массива разного типа.')
            if param.node_type == TypeDesc.INT:
                if flag:
                    self.node_type = TypeDesc.INT_ARRAY
                    flag = False
                elif self.node_type != TypeDesc.INT_ARRAY:
                    self.semantic_error('Элементы массива разного типа.')
            if param.node_type == TypeDesc.FLOAT:
                if flag:
                    self.node_type = TypeDesc.FLOAT_ARRAY
                    flag = False
                elif self.node_type != TypeDesc.FLOAT_ARRAY:
                    self.semantic_error('Элементы массива разного типа.')
            if param.node_type == TypeDesc.STR:
                if flag:
                    self.node_type = TypeDesc.STR_ARRAY
                    flag = False
                elif self.node_type != TypeDesc.STR_ARRAY:
                    self.semantic_error('Элементы массива разного типа.')


class ArrayUpdateNode(StmtNode):
    def __init__(self, var: IdentNode, val: ArrNode,
                 row: Optional[int] = None, line: Optional[int] = None, **props):
        super().__init__(row=row, line=line, **props)
        self.var = var
        self.val = val
    ...
    @property
    def childs(self) -> Tuple[IdentNode, ArrNode]:
        return self.var, self.val
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        if self.var.node_type != self.val.node_type:
            self.semantic_error(
                'Тип {} не совпадает с типом переменной {}'.format(self.val.node_type, self.var.node_type))
        var_ = scope.get_ident(self.var.name)
        if len(self.val.params) != var_.array_num:
            self.semantic_error(
                'Количество элементов {} не совпадает с заданным {}'.format(len(self.val.params), var_.array_num))


class ArrayGetElementNode(StmtNode):
    def __init__(self, var: IdentNode, num: LiteralNode,
                 row: Optional[int] = None, line: Optional[int] = None, **props):
        super().__init__(row=row, line=line, **props)
        self.var = var
        self.num = num
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        self.var.semantic_check(scope)
        self.num.semantic_check(scope)
        if self.var.node_type == TypeDesc.BOOL_ARRAY:
            self.node_type = TypeDesc.BOOL
        elif self.var.node_type == TypeDesc.INT_ARRAY:
            self.node_type = TypeDesc.INT
        elif self.var.node_type == TypeDesc.STR_ARRAY:
            self.node_type = TypeDesc.STR
        elif self.var.node_type == TypeDesc.FLOAT_ARRAY:
            self.node_type = TypeDesc.FLOAT
        if self.num.node_type == TypeDesc.BOOL or self.num.node_type == TypeDesc.STR or self.num.node_type == TypeDesc.FLOAT:
            self.semantic_error(
                'Номер элемента {} может иметь только тип INT'.format(self.num.value))
        if self.num.value >= self.var.node_ident.array_num or self.num.value < 0:
            self.semantic_error(
                'Номер поиции элемента {} не совпадает с размером массива {}'.format(self.num.value,
                                                                                     self.var.node_ident.array_num))

class ArraySetElementNode(StmtNode):
    def __init__(self, var: ArrayGetElementNode, val: ExprNode,
                 row: Optional[int] = None, line: Optional[int] = None, **props):
        super().__init__(row=row, line=line, **props)
        self.var = var
        self.val = val
    ...
    @property
    def childs(self) -> Tuple[ArrayGetElementNode, ExprNode]:
        return self.var, self.val
    ...
    def semantic_check(self, scope: IdentScope) -> None:
        ...
        if self.var.node_type != self.val.node_type:
            self.semantic_error(
                'Тип {} не совпадает с типом элементов массива {}'.format(self.val.node_type, self.var.node_type))
