...
class NotBaseType(Enum):
    """Перечисление для сложных типов данных
    """
...
INT_ARRAY, FLOAT_ARRAY, BOOL_ARRAY, STR_ARRAY = NotBaseType.INT_ARRAY, NotBaseType.FLOAT_ARRAY, NotBaseType.BOOL_ARRAY, NotBaseType.STR_ARRAY
...
class TypeDesc:
    ...
    @staticmethod
    def from_not_base_type(not_base_type_: NotBaseType) -> 'TypeDesc':
        return getattr(TypeDesc, not_base_type_.name)

    @staticmethod
    def from_str(str_decl: str, is_base: bool) -> 'TypeDesc':
        try:
            if is_base:
                base_type_ = BaseType(str_decl)
                return TypeDesc.from_base_type(base_type_)
            else:
                type_ = NotBaseType(str_decl + " array")
                return TypeDesc.from_not_base_type(type_)
        except:
...
for not_base_type in NotBaseType:
    setattr(TypeDesc, not_base_type.name, TypeDesc(not_base_type))
...
class IdentDesc:
    ...
    def __init__(self, name: str, type_: TypeDesc, scope: ScopeType = ScopeType.GLOBAL, array_num : int = None, index: int = 0) -> None:
        self.name = name
        self.type = type_
        self.scope = scope
    ...
    def __str__(self) -> str:
        if self.array_num is None:
            return '{}, {}, {}'.format(self.type, self.scope, 'built-in' if self.built_in else self.index)
        else:
            return '{}, {}, {}, {}'.format(self.type, self.scope, self.array_num, 'built-in' if self.built_in else self.index)
